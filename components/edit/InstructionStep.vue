<template>
  <div class="mt-4 flex flex-col bg-white ring-1 ring-gray-200 rounded-2xl overflow-hidden">
    <div class="flex justify-between items-center">
      <input ref="titleRef" type="text" name="title" id="title" v-model="step.title" :placeholder="`Schritt ${index}`"
        @keyup.enter="onPressEnter"
        class="block w-full text-lg border-0 outline-none px-4 py-3 text-gray-900 placeholder:text-stone-600 sm:leading-6" />

      <button v-if="deleteable" type="button" class="mr-4 h-8 w-8 cursor-pointer text-gray-400 hover:text-gray-600"
        @click="emit('delete')">
        <XMarkIcon />
      </button>
    </div>

    <textarea ref="contentRef" @input="resizeContent" rows="4" name="comment" v-model="step.content" id="comment"
    ...
    <div v-if="data.image" class="p-4">
      <img v-if="typeof data.image.source === 'string'" class="w-full h-full object-cover rounded-lg"
        :src="data.image.source" />

      <input v-model="data.image.caption" placeholder="Bildbeschreibung (optional)" class="w-full py-1"
        @keyup.enter="focusTitleRef" />
    </div>

    <div v-if="data.timer" class="px-4 pb-4">
      <EditCountdownTimer :timer="data.timer" @update:timer="onUpdateTimer" @remove="onRemoveTimer" />
    </div>
  </div>
</template>
